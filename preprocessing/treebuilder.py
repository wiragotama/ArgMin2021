...
class TreeBuilder:
	...
	def get_adj_matrix_discourse_unit(self):
		"""
		:return: numpy.ndarray: adjacency matrix representation of the structure, in discourseunit.py style
		"""
		n = len(self.adj_matrix)
		retval = np.zeros((n, n), dtype="<U5")
		for i in range(len(retval)):
			for j in range(len(retval[i])):
				if self.adj_matrix[i][j] == 1:
					retval[i][j] = "REL_PRESENT"
				else:
					retval[i][j] = NO_REL_SYMBOL
		return retval
	...
	def auto_component_labels(self, AC_breakdown=False, mask_MC=False):
		...
		for i in range(n):
			...
			if n_incoming > 0 and n_outgoing == 0:
				...
				if AC_breakdown:
					labels.append("AC (leaf)")
				else:
					labels.append("AC")
			else:
				if AC_breakdown:
					labels.append("AC (non-leaf)")
				else:
					labels.append("AC")
		return labels
	...
	def is_tree(self):
		...
		if n_cand == 1: # there is only one root candidate
			...
			if not verdict: # no cycle
				...
				flag = True
				for i in range(n):
					if not visited[i] and component_labels[i] != "non-AC":
						flag = False
						break
				return flag
			else:
